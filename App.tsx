...
import { Loader2, X, Code2, ImageIcon, FileJson } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleImageSelect = async (base64: string) => {
    ...
    // Auto-start JSON analysis if on JSON tab, or let user trigger it? 
    // Let's trigger based on active tab to be helpful, or just wait.
    // For now, let's trigger JSON automatically as that was the original behavior
    if (activeTab === 'json') {
        analyzeJson(base64);
    }
  };
  ...
  const handleTabChange = (tab: 'json' | 'pose') => {
    setActiveTab(tab);
    // Optional: Auto-trigger if data is missing and image exists
    if (uploadedImage) {
        if (tab === 'json' && !promptData && jsonState === AppState.IDLE) {
            analyzeJson(uploadedImage);
        } else if (tab === 'pose' && !poseImage && poseState === AppState.IDLE) {
            // We can auto-trigger pose too, or let user click. 
            // Let's let user click for pose to save credits/time, 
            // but the prompt implied "create a separate function".
            // I'll leave it manual button for Pose to be safe, or auto?
            // "Generate image that simply shows pose... separate from current".
            // I'll show a button in the pose tab.
        }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-4 md:p-6 lg:p-8 flex flex-col gap-6">
        ...
        {/* Global Error (if any critical one needed, otherwise local errors are better) */}
                        ...
                        {promptData ? (
                            ...
                            <div className="h-full rounded-xl border-2 border-dashed border-gray-800 bg-gray-900/30 flex flex-col items-center justify-center text-center p-8 text-gray-600">
                                ...
                                {uploadedImage && jsonState === AppState.IDLE && (
                                     <button 
                                        onClick={() => analyzeJson(uploadedImage)}
                        ...
                        {poseImage ? (
                            <PoseDisplay imageUrl={poseImage} />
                             ...
                             <div className="h-full rounded-xl border-2 border-dashed border-gray-800 bg-gray-900/30 flex flex-col items-center justify-center text-center p-8 text-gray-600">
                                ...
                                {uploadedImage ? (
                                    <button 
                                        onClick={() => generatePose(uploadedImage)}
                                        disabled={poseState === AppState.ANALYZING}
                                        className="mt-4 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-lg transition-all shadow-lg hover:shadow-indigo-500/25 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                                    >
